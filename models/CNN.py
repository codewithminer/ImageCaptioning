...
class EncoderCNN(nn.module):
    """ Load the pretrained ReNet-152 and replace top fc layer."""
    def __init__(self, embed_size):
        super(EncoderCNN, self).__init__()
        resnet = models.resnet152(pretrained=True)
        modules = list(resnet.children()[:-1]) # It “chains” outputs to inputs sequentially for each subsequent module, finally returning the output of the last module.
        self.resnet = nn.Sequential(*modules)
        self.bn = nn.BatchNorm1d(embed_size, momentum=0.01)

    def forward(self, images):
        """Extract feature vectors from input image"""
        with torch.no_grad():
...
class DecoderRNN(nn.Module):
    def __init__(self, embed_size, hidden_size, vocab_size, num_layer, max_seq_length=20):
        """ Set the hyper-parameters and build the layer"""
    ...
    def forward(self, features, captions, lengths):
        """ Decode image feature vector and generate caption"""
