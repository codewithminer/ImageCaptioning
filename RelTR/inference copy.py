...
def main(args):
    ...
    dataset_path = '/root/Parseh/Datasets/MyDataSet(images-contains-COCO-objects)'
    IMAGE_PATHS = []
    for folder_name in os.listdir(dataset_path):
        for filename in glob.iglob(os.path.join(dataset_path, folder_name, '*.jpg'), recursive=True):
            IMAGE_PATHS.append(filename)
    
    #for filename in glob.glob(f'{dataset_path}/*/**/*.jpg', recursive=True):
    #    IMAGE_PATHS.append(filename)
    print('Numebr of images in dataset:', len(IMAGE_PATHS))
    ...
    objs_bbox_reg = []
    objs_label_reg = []
    im = Image.open('/root/Parseh/RelTR/images/img2.jpg')
        ...
        if len(keep_queries) > 0:
            ...
            with torch.no_grad():
                ...
                for idx, ax_i, (sxmin, symin, sxmax, symax), (oxmin, oymin, oxmax, oymax) in \
                    ...
                    ax.axis('off')
                    ax.set_title(CLASSES[probas_sub[idx].argmax()]+' '+
                                 REL_CLASSES[probas[idx].argmax()]+' '+
                                 CLASSES[probas_obj[idx].argmax()], fontsize=10)
                    ...
                    #rels.append((CLASSES[probas_sub[idx].argmax()]+ '(' + str(s_id) + ')' + ' '
                    #            +REL_CLASSES[probas[idx].argmax()]+' '
                    #            +CLASSES[probas_obj[idx].argmax()] +  '(' + str(o_id) + ')'))
                ...
                scene_graphs.append(graph_dict)
    fig.tight_layout()
    plt.show()
